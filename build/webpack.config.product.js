...
let config = {
    ...
    resolve: {
        ...
        alias: { //给对应的模块取别名:不会影响文件打包大小，不会因为别名而导致dll重复加载对应的库
            ...
            /*vue.runtime.esm.js是运行时版本，不带模板编译器；而vue.esm.js是完整版本，包含了模板编译器，再打包静态资源的时候，需要用到完整的版本用于编译vue中的模板，所以用vue.esm.js
              结果webpack.config.prod.js打包完成以后，生成的静态文件的vue已经通过了编译，但是不需要把vue整个文件打包进js，所以插件中用了webpack.DllReferencePlugin，遇到vue自动忽略
              这样依赖，只要自己在html中添加对应的包含vue的dll.js文件即可；dll.js文件里面用的是运行时的vue，也就是vue.runtime.esm.js
             */
